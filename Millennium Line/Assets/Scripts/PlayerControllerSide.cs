...
public class PlayerControllerSide : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (rb.velocity.x < 0)
        {
            ...
            spriteAnimator.transform.GetChild(0).eulerAngles = Vector3.up * 180;
            spriteAnimator.SetBool("Moving", true);
        }
        else if (rb.velocity.x > 0)
        {
            spriteAnimator.GetComponent<SpriteRenderer>().flipX = false;
            spriteAnimator.transform.GetChild(0).eulerAngles = Vector3.zero;
            spriteAnimator.SetBool("Moving", true);
        }
    ...
    float GroundCheck()
    {
        RaycastHit2D downwardHitRight = Physics2D.Raycast(groundRaycastPoint.position + Vector3.right * GetComponent<CapsuleCollider2D>().size.x / 2f, 
            Vector2.down, 100f, groundLayerMask);
        RaycastHit2D downwardHitLeft = Physics2D.Raycast(groundRaycastPoint.position - Vector3.right * GetComponent<CapsuleCollider2D>().size.x / 2f,
            Vector2.down, 100f, groundLayerMask);

        return Mathf.Min(Vector2.Distance(downwardHitLeft.point, groundRaycastPoint.position), 
            Vector2.Distance(downwardHitRight.point, groundRaycastPoint.position));
    }

    float LadderCheck()
    {
        RaycastHit2D downwardHitRight = Physics2D.Raycast(groundRaycastPoint.position + Vector3.right * GetComponent<CapsuleCollider2D>().size.x / 2f,
            Vector2.down, 100f, ladderLayerMask);
        RaycastHit2D downwardHitLeft = Physics2D.Raycast(groundRaycastPoint.position - Vector3.right * GetComponent<CapsuleCollider2D>().size.x / 2f,
            Vector2.down, 100f, ladderLayerMask);
    ...
    void TryVault(bool climb)
    {
        ...
        {
            Vector2 direction = spriteAnimator.GetComponent<SpriteRenderer>().flipX ? Vector2.left : Vector2.right;
            RaycastHit2D downwardHit = Physics2D.Raycast(new Vector2(transform.position.x + direction.x * climbCheckDistance, groundRaycastPoint.transform.position.y + maxClimbHeight), Vector2.down, 10f, collisionLayerMask);
            Debug.DrawRay(new Vector2(transform.position.x + direction.x * climbCheckDistance, groundRaycastPoint.transform.position.y + maxClimbHeight), Vector2.down, Color.red, 1f);
